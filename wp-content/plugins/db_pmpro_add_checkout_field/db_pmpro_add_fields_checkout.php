<?php
/*
Plugin Name: PMPRO Add fields to checkout and custom checkout shortcode
...
function my_pmprorh_init()
{
	...
	$fields[] = new PMProRH_Field(
		'anniversary',						// input name, will also be used as meta key
		'date',							// type of field
		array(
			'label'		=> ' '	,		// custom field label
			'size'		=> 10,				// input size
			'class'		=> 'anniversary',			// custom class
	...
	$fields[] = new PMProRH_Field(
		'anniversary-tool',						// input name, will also be used as meta key
		'hidden',							// type of field
		array(
			'value'		=> 1,		// value
...
*/
function db_pmprorh_signup_shortcode($atts, $content=null, $code="")
{
	...
	?>
		<?php if(!empty($current_user->ID) && pmpro_hasMembershipLevel($level,$current_user->ID)) { ?>
			...
			<a class="button primary-btn" href="<?php echo get_home_url() . '/user-profile/'; ?>"><?php _e('Go to account page','pmpro'); ?></a>
		<?php } else { ?>
		<form class="pmpro_form pmpro_signup_form" action="<?php echo pmpro_url("checkout"); ?>" method="post">
			...
			<?php
				...
				else
				{
					...
					<div>
						...
						<input id="username" name="username" type="text" class="input" size="30" value="" placeholder="Pick a username"/>
					</div>
					<?php do_action("pmpro_checkout_after_username");?>
					<div>
						<!--<label for="password">Password</label>-->
						<input id="password" name="password" type="password" class="input" size="30" value="" placeholder="Create a password"/>
					</div>
					<?php if($short) { ?>
					...
					<div>
						...
						<input id="bemail" name="bemail" type="email" class="input" size="30" value="" placeholder="Your email address"/>
					</div>
					<?php if($short) { ?>
			...
			<div>
				<span id="pmpro_submit_span" >
					...
					<input type="submit" class="button primary-btn" value="<?php echo $button; ?>" />
				</span>
			</div>
			<?php if(!empty($login) && empty($current_user->ID)) { ?>
			<div class="hidden-login-on-shortcode" style="text-align:center;">
...
add_shortcode("db_pmpro_signup", "db_pmprorh_signup_shortcode");
