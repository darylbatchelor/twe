...
class Layers_Pro_Widget_Filters {
	...
	public function init(){
		...
		if ( !version_compare( $this->layers_meta->get( 'Version' ), '1.5.0', '<' ) ) {
			...
			// StoreKit
			// add_filter( 'layers_design_bar_fonts_product-categories_elements', array( $this, 'add_heading_type' ), 10, 2 );
			// add_filter( 'layers_design_bar_fonts_product_elements', array( $this, 'add_heading_type' ), 10, 2 );
			// add_filter( 'layers_design_bar_fonts_product-slider_elements', array( $this, 'add_heading_type' ), 10, 2 ); // Disabled for now.
	...
	*/
	public function add_heading_type( $elements, $widget ){
		...
		$new_elements = array(
			...
			/*
			'fonts-heading-type-two' => array(
				'type' => 'select-icons',
				'label' => __( 'Heading Type Two', 'layerswp' ),
				'name' => $widget->get_layers_field_name( 'fonts', 'heading-type-two' ),
				'id' => $widget->get_layers_field_id( 'fonts', 'heading-type-two' ),
				'value' => ( isset( $widget->values['fonts']['heading-type-two'] ) ) ? $widget->values['fonts']['heading-type-two'] : NULL,
				'options' => array(
					'h1' => array( 'name' => __( 'H1', 'layerswp' ), 'class' => 'icon-heading-1', 'data' => '' ),
					'h2' => array( 'name' => __( 'H2', 'layerswp' ), 'class' => 'icon-heading-2', 'data' => '' ),
					'h3' => array( 'name' => __( 'H3', 'layerswp' ), 'class' => 'icon-heading-3', 'data' => '' ),
	...
	*/
	...
	public function slider_video_bg( $widget_this, $item_instance = array(), $widget_instance = array()){
		...
		<?php }
		elseif( function_exists( 'layers_get_youtube_id' ) && ( $youtube && 'youtube' == $bg_video_type ) || ( $vimeo && 'vimeo' == $bg_video_type ) ) {
			if( !$wp_customize && 1 == count( $widget_this->check_and_return( $widget_instance, 'slides' ) ) ) {
				$autoplay = '&autoplay=1';
			} else {
				$autoplay = '';
			} ?>
			<div class="layerspro-slider-video">
				<?php if( $youtube && 'youtube' == $bg_video_type ) {
					$id = layers_get_youtube_id( $youtube ) ?>
					<iframe frameborder="0" height="100%" width="100%" src="//www.youtube.com/embed/<?php echo $id; ?>?enablejsapi=1&controls=0&loop=1&rel=0&showinfo=0&autohide=1&wmode=transparent&hd=1<?php echo $autoplay; ?>"></iframe>
				<?php } elseif( $vimeo && 'vimeo' == $bg_video_type ) {
					$id = layers_get_vimeo_id( $vimeo ); ?>
					<iframe frameborder="0" height="100%" width="100%" src="//player.vimeo.com/video/<?php echo $id; ?>?title=0&controls=0&byline=0&portrait=0&loop=1<?php echo $autoplay; ?>" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
				<?php }  ?>
			</div>
		<?php }
	}
