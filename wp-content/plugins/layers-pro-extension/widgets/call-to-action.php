					...
					<?php if( ! empty( $instance['buttons'] ) ) { ?>
						...
						<div class="<?php echo esc_attr( $classes ); ?>">
							<?php foreach ( explode( ',', $instance['button_ids'] ) as $item_key ) {
								...
								// Hover styling (to be removed).
								/*if ( ! empty( $item_instance['design']['buttons-background-color'] ) ) {

									$background       = $item_instance['design']['buttons-background-color'];
									$background_hover = layers_too_light_then_dark( $background, 20 );
									$color            = layers_get_light_or_dark( $background, '#454545', '#ffffff' ); // Light, Dark.

									// Apply button hover
									$this->inline_css .= layers_inline_styles( "#{$widget_id} #{$item_id_attr}.button:hover", array( 'css' => array(
